...
function PokemonItem(props: { pokemonName: string, url: string}) {
    ...
    if (isPending) return <div className='pokemonTile group/pokemonItem select-none hover:scale-105'><div className="pokemonTextbox rounded-lg md:rounded-xl flex flex-col relative">Searching...</div></div>;
    if (isError) return <div className='pokemonTile group/pokemonItem select-none hover:scale-105'><div className="pokemonTextbox rounded-lg md:rounded-xl flex flex-col relative">Error Loading</div></div>
    ...
    return (
        <a href={'/' + props.pokemonName}>
            <div className='pokemonTile group/pokemonItem select-none hover:scale-105'>
                <div className="h-12 md:h-14 relative z-[10]">
                    <img className={'pokemonImage aspect-square object-contain overflow-visible absolute sm:top-0 sm:left-1/2 sm:-translate-x-1/2 ' + (listType === 3 ? '-left-1.5 top-3 w-16 sm:w-20 md:w-28' : 'left-1/2 -translate-x-1/2 w-24 sm:w-28 md:w-32') } src={data.image} />
                </div>
