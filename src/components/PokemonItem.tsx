...
function PokemonItem(props: { itemData: pokedexDataForm, key: number }) {
    ...
    const pokemonTypes = pokemonDisplay.types.map((pokemonType: string) => {
        ...
        return (<img key={pokemonDisplay.id} className='w-4 p-0.5 md:w-6 md:p-1 mt-1 rounded-full' style={{backgroundColor: typeDataEntry.color}} src={typeDataEntry.icon} />)
    });
    return (
        <div className='pokemonTile select-none'>
            <div className="h-12 md:h-14 relative z-[10]">
                <img className='pokemonImage rounded-full aspect-square w-20 md:w-28 object-contain overflow-visible absolute left-1/2 -translate-x-1/2' src={pokemonDisplay.image} />
            </div>
            <div className="pokemonTextbox rounded-xl flex flex-col relative">
                <div className="absolute ps-1">
                    {pokemonTypes}
                </div>
                <p className='text-sm md:text-lg pe-2 noto-sans-400 self-end'>{pokemonDisplay.id}</p>
