...
function PokemonView() {
    ...
    const pokemonTypes = !(assetData.types === undefined) ? assetData.types.map((pokemonType: string) => {
        ...
        return (<img key={assetData.name} className='w-8 p-1.5 md:w-12 md:p-2 mt-1 object-scale-down object-center rounded-full' style={{backgroundColor: typeDataEntry.color}} src={typeDataEntry.icon} />)
    }): [];
    return (
        <div className='my-auto mx-4 md:mx-20 h-[50vh] z-10'>
            <div className="pokemonDisplay h-full mt-[3vh] px-4">
                {fetchAssets && (
                    <>
                        ...
                        <a href="../">
                            <img className='pokemonBackButton rounded-full w-10 h-10 aspect-square bg-white bg-opacity-25 p-1 mt-4 hover:bg-opacity-1 hover:bg-[#dd0b2d]' src={backIcon} />
                        </a>
